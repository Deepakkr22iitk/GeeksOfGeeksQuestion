...
class Solution {
  public:
    ...
    {
        // if in any dirn the index goes out of the mat
        if(i>=mat.size() || j>=mat[0].size()) return nullptr;
        
        // creating a new Node for the current indexes
        Node* curr=new Node(mat[i][j]);
        
        // creating the row and the column for the current cell
